---
name: ocr-openvino-conversion
description: >
  Convert PaddleOCR PP-OCRv5 models to OpenVINO for GPU inference in the OCR
  service. Covers why the PaddleX JSON format requires an intermediate ONNX
  step (paddle2onnx), the model names downloaded for Spanish (lang="es"),
  build-time conversion in convert_models.py, and runtime loading with
  OpenVINO on Intel GPU. Use when working on the ocr-service, model conversion,
  paddle2onnx, OpenVINO compile/read_model, ONNX export, or PaddleOCR model
  download. Triggers on: paddleocr, openvino, paddle2onnx, onnx conversion,
  ocr model, PP-OCRv5, intel gpu inference, ovms.
---
