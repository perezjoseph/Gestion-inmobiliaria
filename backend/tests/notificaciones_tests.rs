#[cfg(test)]
mod notificaciones_handler_tests {
    ...
    #[actix_web::test]
    async fn pagos_vencidos_rejects_unauthenticated_request() {
        let app = test::init_service(App::new().app_data(web::Data::new(test_config())).route(
            "/api/notificaciones/pagos-vencidos",
    ...
    #[actix_web::test]
    async fn pagos_vencidos_allows_any_authenticated_role() {
        let app = test::init_service(App::new().app_data(web::Data::new(test_config())).route(
            "/api/notificaciones/pagos-vencidos",
            web::get().to(claims_stub),
        ))
