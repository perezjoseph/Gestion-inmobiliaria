...
pub async fn ingresos_comparacion(
    ...
    let result = dashboard::ingreso_comparacion(db.get_ref(), claims.organizacion_id).await?;
    Ok(HttpResponse::Ok().json(result))
}
...
pub async fn pagos_proximos(
    ...
    let result = dashboard::pagos_proximos(db.get_ref(), claims.organizacion_id, dias).await?;
    Ok(HttpResponse::Ok().json(result))
}
...
pub async fn contratos_calendario(
    ...
    let result = dashboard::contratos_calendario(db.get_ref(), claims.organizacion_id).await?;
    Ok(HttpResponse::Ok().json(result))
}
...
pub async fn gastos_comparacion(
    ...
    let result = dashboard::gastos_comparacion(db.get_ref(), claims.organizacion_id).await?;
    Ok(HttpResponse::Ok().json(result))
}
