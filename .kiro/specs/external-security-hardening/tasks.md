...
- [ ] 1. Fix trusted client IP extraction in rate limiter
  - [ ] 1.1 Update `FallbackPeerIpKeyExtractor` in `backend/src/middleware/rate_limit.rs`
...
- [ ] 2. Implement account lockout on failed login
  - [ ] 2.1 Add `dashmap` dependency to `backend/Cargo.toml`
    - Research latest stable version and pin it
    - _Requirements: 2.6_

  - [ ] 2.2 Create `backend/src/services/login_lockout.rs`
