...
- Help the user evaluate the results both qualitatively and quantitatively
  - While the runs happen in the background, draft some quantitative evals if there are not any. Then explain them to the user
  - Use the eval-viewer/generate_review.py script to show the user the results
- Rewrite the skill based on feedback from the user evaluation of the results
...
- **Triggering**: Kiro activates skills via discloseContext based on the skill 
ame and description fields in YAML frontmatter
...
- **No claude CLI for trigger evals**: The 
un_eval.py and 
un_loop.py scripts depend on claude -p. For Kiro, trigger evaluation is done manually (see Description Optimization section).
...
Start by understanding the user intent. The current conversation might already contain a workflow the user wants to capture. If so, extract answers from the conversation history first.
...
Proactively ask questions about edge cases, input/output formats, example files, success criteria, and dependencies. Wait to write test prompts until you have got this part ironed out.
...
After writing the skill draft, come up with 2-3 realistic test prompts. Share them with the user and ask for confirmation.

Save test cases to evals/evals.json. Do not write assertions yet, just the prompts.
...
See 
eferences/schemas.md for the full schema.
...
Save timing data to 	iming.json in the run directory:
...
1. **Grade each run** by spawning a grader sub-agent (or grade inline) that reads gents/grader.md and evaluates each assertion against the outputs. Save results to grading.json. The expectations array must use the fields 	ext, passed, and evidence.

2. **Aggregate into benchmark** by running the aggregation script from the skill-creator directory:

   `powershell
   python -m scripts.aggregate_benchmark <workspace>/iteration-N --skill-name <name>
   `

   This produces enchmark.json and enchmark.md. See 
eferences/schemas.md for the exact schema.

3. **Do an analyst pass** by reading the benchmark data and surfacing patterns. See gents/analyzer.md for what to look for.
...
5. **Tell the user**: "I have opened the results in your browser. There are two tabs: Outputs lets you click through each test case and leave feedback, Benchmark shows the quantitative comparison. When you are done, come back here and let me know."
...
- **Previous Output** (iteration 2+): collapsed section showing last iteration output
...
When the user tells you they are done, read eedback.json:
...
1. **Generalize from the feedback.** We are trying to create skills that work across many prompts. Rather than put in fiddly overfitty changes, try branching out and using different metaphors or patterns.

2. **Keep the prompt lean.** Remove things that are not pulling their weight. Read the transcripts, not just the final outputs.

3. **Explain the why.** Try hard to explain the **why** behind everything you are asking the model to do. If you find yourself writing ALWAYS or NEVER in all caps, reframe and explain the reasoning instead.

4. **Look for repeated work across test cases.** If all test cases resulted in the sub-agent writing a similar script, that is a signal the skill should bundle that script in scripts/.
...
For situations where you want a more rigorous comparison between two versions of a skill, there is a blind comparison system. Read gents/comparator.md and gents/analyzer.md for the details.

This is optional and most users will not need it.
...
Skills appear in Kiro Available Items list with their 
ame and description. When the user sends a message, Kiro decides whether to call discloseContext to activate a skill based on keyword/intent matching against the description.

Key insight: Kiro triggering is keyword-and-intent based. The description should contain the key terms a user would naturally use when they need this skill.
...
Since Kiro does not have a claude -p equivalent for automated trigger testing, evaluate triggering manually:

1. For each query, assess whether the skill current description contains enough keyword overlap and intent signal that Kiro would activate it
2. Check for false-positive risk: would the description cause activation on the should-not-trigger queries?
3. Look for gaps: are there should-trigger queries whose key terms do not appear anywhere in the description?
...
Update the skill SKILL.md frontmatter description. Show the user before/after and explain what changed and why.
...
- gents/grader.md - How to evaluate assertions against outputs
- gents/comparator.md - How to do blind A/B comparison between two outputs
- gents/analyzer.md - How to analyze why one version beat another

The references/ directory has additional documentation:

- 
eferences/schemas.md - JSON structures for evals.json, grading.json, etc.
