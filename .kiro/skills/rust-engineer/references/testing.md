...
Tests introduce unique deadlock risks because Rust's test harness runs tests in parallel by default and tests often share global state.

### Shared `static Mutex` for env vars

Tests that manipulate environment variables use a `static Mutex<()>` to serialize access. If a test panics while holding the lock, the Mutex becomes poisoned and all subsequent tests that call `.lock().unwrap()` also panic in a cascade.
...
#[test]
fn test_env_var() {
    // Recover from poison so one panicking test doesn't kill the rest
...
### Parallel tests hitting the same database

Multiple integration tests running concurrently against the same database can deadlock when their transactions lock the same rows in different orders. PostgreSQL will detect and abort one transaction, but the test still fails unpredictably.

```rust
// Option 1: use serial_test crate
...
// Option 2: run all tests single-threaded
// cargo test -- --test-threads=1
```

### `#[tokio::test]` single-threaded runtime

The default `#[tokio::test]` uses a `current_thread` runtime. If the test spawns multiple tasks that contend on a shared resource, they can't make progress because there's only one thread.

```rust
// DEADLOCK: two tasks contend on a lock, single thread can't drive both
#[tokio::test]
async fn test_contention() {
    let data = Arc::new(tokio::sync::Mutex::new(0));
    // task A holds lock, yields, task B needs lock, blocks the only thread
}

// FIX: use multi_thread flavor
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_contention() {
    // Now tasks can run on separate threads
}
```

### Blocking in async test context

Calling `.lock()` on a `std::sync::Mutex` inside an async test blocks the executor thread. If another task on the same thread holds that lock, deadlock. Use `tokio::sync::Mutex` or `spawn_blocking` instead.

### Cleanup holding resources

A test's `Drop` impl or cleanup function tries to acquire a lock or DB connection that the test itself still holds. Scope guards and connections tightly — drop them before cleanup runs.
...
- Use `.lock().unwrap_or_else(|e| e.into_inner())` for static Mutex in tests to survive poison
- Use `#[serial]` or `--test-threads=1` for tests that share a database
- Use `#[tokio::test(flavor = "multi_thread")]` when tests have lock contention
