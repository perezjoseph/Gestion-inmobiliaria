...
/// Compares linear scan vs pre-indexed HashMap lookup.
...
/// Benchmark combined filter: contrato_id + estado (common in payments list page).
...
/// Benchmark fallback case: no contrato_id filter (both should be similar).
...
/// Benchmark date range filter with contrato_id.
...
/// Benchmark index construction cost (one-time amortized cost).
...
/// Benchmark with varying dataset sizes to show scaling behavior.
