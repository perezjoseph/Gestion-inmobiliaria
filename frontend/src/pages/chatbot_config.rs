...
fn StepperNav(props: &StepperNavProps) -> Html {
    ...
    html! {
        <div
            ...
            class="flex items-center gap-0 mb-6 overflow-x-auto pb-2"
...
fn StepTab(props: &StepTabProps) -> Html {
    let circle_class = if props.is_active {
        "w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold bg-[var(--color-primary-500)] text-white shrink-0"
    } else if props.is_completed {
        "w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold bg-emerald-100 text-emerald-700 shrink-0"
    } else {
        "w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold bg-[var(--surface-raised)] text-[var(--text-tertiary)] border border-[var(--border-default)] shrink-0"
    };

    let label_class = if props.is_active {
        "text-xs font-medium text-[var(--text-primary)] whitespace-nowrap"
    } else if props.is_completed {
        "text-xs font-medium text-emerald-700 whitespace-nowrap"
    } else {
        "text-xs text-[var(--text-tertiary)] whitespace-nowrap"
    };

    let line_class = if props.is_completed {
        "flex-1 h-0.5 bg-emerald-400 min-w-[1rem]"
    } else {
        "flex-1 h-0.5 bg-[var(--border-default)] min-w-[1rem]"
    };
    ...
    html! {
        <>
            <button
                ...
                class="flex flex-col items-center gap-1 px-2 py-1 cursor-pointer bg-transparent border-none outline-none focus-visible:ring-2 focus-visible:ring-[var(--border-focus)] rounded"
            >
