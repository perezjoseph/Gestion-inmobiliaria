...
fun PropiedadEntity.toDomain(): Propiedad = Propiedad(
    id = id,
    titulo = titulo,
...
fun PropiedadDto.toEntity(): PropiedadEntity = PropiedadEntity(
    id = id,
    titulo = titulo,
    descripcion = descripcion,
    direccion = direccion,
...
fun Propiedad.toCreateRequest(): CreatePropiedadRequest = CreatePropiedadRequest(
    titulo = titulo,
    descripcion = descripcion,
    direccion = direccion,
    ciudad = ciudad,
    ...
    moneda = moneda,
    estado = estado,
    imagenes = if (imagenes.isNotEmpty()) {
        JsonArray(imagenes.map { JsonPrimitive(it) })
    } else null
)

private fun parseImagenesJson(json: String): List<String> = try {
