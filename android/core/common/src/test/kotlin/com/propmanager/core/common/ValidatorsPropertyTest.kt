 ...
 */
class ValidatorsPropertyTest : FreeSpec({
    ...
    fun blankFieldArb(fieldCount: Int, decimalIndices: Set<Int> = emptySet()): Arb<Pair<Int, Array<String>>> =
        arbitrary {
            val idx = Arb.int(0 until fieldCount).bind()
            val fields = Array(fieldCount) { i ->
                if (i == idx) {
                    blankArb.bind()
    ...
    "Property 10: Entity form validation rejects blank required fields" - {
        ...
        "PropiedadValidator rejects any blank required field" {
            checkAll(100, blankFieldArb(6, setOf(5))) { (blankIdx, fields) ->
                val result = PropiedadValidator.validateCreate(
                    titulo = fields[0],
                    direccion = fields[1],
                    ciudad = fields[2],
                    provincia = fields[3],
        ...
        "InquilinoValidator rejects any blank required field" {
            checkAll(100, blankFieldArb(3)) { (blankIdx, fields) ->
                val result = InquilinoValidator.validateCreate(
                    nombre = fields[0],
                    apellido = fields[1],
                    cedula = fields[2],
                )
        ...
        "ContratoValidator rejects any blank required field" {
            ...
            checkAll(100, contBlankArb) { (blankIdx, fields) ->
                val result = ContratoValidator.validateCreate(
        ...
        "PagoValidator rejects any blank required field" {
            ...
            checkAll(100, pagoBlankArb) { (blankIdx, fields) ->
                val result = PagoValidator.validateCreate(
                    contratoId = fields[0],
                    monto = fields[1],
        ...
        "GastoValidator rejects any blank required field" {
            ...
            checkAll(100, gastoBlankArb) { (blankIdx, fields) ->
                val result = GastoValidator.validateCreate(
                    propiedadId = fields[0],
                    categoria = fields[1],
        ...
        "SolicitudValidator rejects any blank required field" {
            checkAll(100, blankFieldArb(2)) { (blankIdx, fields) ->
                val result = SolicitudValidator.validateCreate(
                    propiedadId = fields[0],
                    titulo = fields[1],
                )
        ...
        "all validators return Valid when all required fields are non-blank" {
            checkAll(100, nonBlankArb, nonBlankArb, posDecimalArb) { s1, s2, decimal ->
                PropiedadValidator.validateCreate(s1, s2, s1, s2, s1, decimal).values
                    .forEach { it.shouldBeInstanceOf<ValidationResult.Valid>() }

                InquilinoValidator.validateCreate(s1, s2, s1).values
                    .forEach { it.shouldBeInstanceOf<ValidationResult.Valid>() }

                PagoValidator.validateCreate(s1, decimal, "2025-07-01").values
                    .forEach { it.shouldBeInstanceOf<ValidationResult.Valid>() }

                GastoValidator.validateCreate(s1, s2, s1, decimal, s2, "2025-06-15").values
                    .forEach { it.shouldBeInstanceOf<ValidationResult.Valid>() }

                SolicitudValidator.validateCreate(s1, s2).values
                    .forEach { it.shouldBeInstanceOf<ValidationResult.Valid>() }

                ContratoValidator.validateCreate(s1, s2, "2025-01-01", "2026-01-01", decimal).values
                    .forEach { it.shouldBeInstanceOf<ValidationResult.Valid>() }
            }
        }
    }
