...
constructor(
    ...
    open fun observeAll(): Flow<List<Inquilino>> =
        dao.observeAll().map { entities -> entities.map { it.toDomain() } }

    open fun search(query: String): Flow<List<Inquilino>> =
    ...
    open suspend fun create(request: CreateInquilinoRequest): Result<Inquilino> = runCatching {
    ...
    open suspend fun update(id: String, request: UpdateInquilinoRequest): Result<Unit> =
        runCatching {
            syncQueueDao.enqueue(
                SyncQueueEntry(
                    entityType = "inquilino",
