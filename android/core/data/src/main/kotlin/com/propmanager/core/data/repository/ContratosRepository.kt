...
constructor(
    ...
    open fun observeAll(): Flow<List<Contrato>> =
    ...
    open fun observeExpiring(daysThreshold: Int = 30): Flow<List<Contrato>> {
        ...
        return dao.observeExpiring(today, threshold).map { entities ->
            entities.map { it.toDomain() }
        }
    }

    open suspend fun create(request: CreateContratoRequest): Result<Contrato> = runCatching {
    ...
    open suspend fun renew(id: String, request: RenovarContratoRequest): Result<Unit> =
        runCatching {
            syncQueueDao.enqueue(
                SyncQueueEntry(
                    entityType = "contrato",
    ...
    open suspend fun terminate(id: String, request: TerminarContratoRequest): Result<Unit> =
        runCatching {
            syncQueueDao.enqueue(
                SyncQueueEntry(
                    entityType = "contrato",
