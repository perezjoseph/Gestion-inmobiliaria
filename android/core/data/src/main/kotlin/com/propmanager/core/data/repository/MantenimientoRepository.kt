...
constructor(
    ...
    open fun observeAll(): Flow<List<SolicitudMantenimiento>> =
    ...
    open fun observeFiltered(
        ...
        solicitudDao.observeFiltered(estado, prioridad, propiedadId).map { entities ->
            entities.map { it.toDomain() }
        }

    open fun observeById(id: String): Flow<SolicitudMantenimiento?> =
    ...
    open suspend fun update(id: String, request: UpdateSolicitudRequest): Result<Unit> =
        runCatching {
            syncQueueDao.enqueue(
                SyncQueueEntry(
                    entityType = "mantenimiento",
    ...
    open suspend fun updateEstado(id: String, request: UpdateEstadoRequest): Result<Unit> =
        runCatching {
            syncQueueDao.enqueue(
                SyncQueueEntry(
                    entityType = "mantenimiento",
    ...
    ): Result<NotaMantenimiento> = runCatching {
        val id = UUID.randomUUID().toString()
        val now = Instant.now().toEpochMilli()
        val entity =
