...
class MantenimientoRepository @Inject constructor(
    ...
    fun observeAll(): Flow<List<SolicitudMantenimiento>> =
        solicitudDao.observeAll().map { entities -> entities.map { it.toDomain() } }
    ...
    fun observeFiltered(
        ...
        solicitudDao.observeFiltered(estado, prioridad, propiedadId)
            .map { entities -> entities.map { it.toDomain() } }

    fun observeById(id: String): Flow<SolicitudMantenimiento?> =
        solicitudDao.observeById(id).map { it?.toDomain() }
    ...
    suspend fun create(request: CreateSolicitudRequest): Result<SolicitudMantenimiento> = runCatching {
        val id = UUID.randomUUID().toString()
        val now = Instant.now().toEpochMilli()
        val entity = SolicitudMantenimientoEntity(
            id = id,
    ...
    suspend fun update(id: String, request: UpdateSolicitudRequest): Result<Unit> = runCatching {
        syncQueueDao.enqueue(
            SyncQueueEntry(
                entityType = "mantenimiento",
                entityId = id,
    ...
    suspend fun updateEstado(id: String, request: UpdateEstadoRequest): Result<Unit> = runCatching {
        syncQueueDao.enqueue(
            SyncQueueEntry(
                entityType = "mantenimiento",
                entityId = id,
    ...
    suspend fun addNota(solicitudId: String, request: CreateNotaRequest): Result<NotaMantenimiento> = runCatching {
        val id = UUID.randomUUID().toString()
        val now = Instant.now().toEpochMilli()
        val entity = NotaMantenimientoEntity(
            id = id,
