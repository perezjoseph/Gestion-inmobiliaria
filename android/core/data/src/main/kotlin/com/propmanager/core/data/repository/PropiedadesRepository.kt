...
constructor(
    ...
    open fun observeAll(): Flow<List<Propiedad>> =
    ...
    open fun observeFiltered(
        ...
        dao.observeFiltered(ciudad, estado, tipoPropiedad).map { entities ->
            entities.map { it.toDomain() }
        }

    open fun observeById(id: String): Flow<Propiedad?> = dao.observeById(id).map { it?.toDomain() }

    open suspend fun create(request: CreatePropiedadRequest): Result<Propiedad> = runCatching {
    ...
    open suspend fun update(id: String, request: UpdatePropiedadRequest): Result<Unit> =
        runCatching {
            val now = Instant.now().toEpochMilli()
            syncQueueDao.enqueue(
                SyncQueueEntry(
