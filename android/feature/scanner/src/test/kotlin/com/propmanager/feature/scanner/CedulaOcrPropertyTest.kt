 ...
 *
 * For any text block following the Dominican cédula layout pattern (containing a formatted cédula
 * number, nombre, and apellido in expected positions), CedulaOcrExtractor.parseCedulaLines()
 * extracts a CedulaOcrResult where the cedula, nombre, and apellido fields match the values present
 * in the input text.
 */
class CedulaOcrPropertyTest :
    FreeSpec({
        ...
        val cedulaNumberArb: Arb<String> = arbitrary {
        ...
        val dominicanNameArb: Arb<String> =
            arbitrary(edgecases = listOf("Juan", "María", "José", "Ana")) {
                val names =
                    listOf(
                        "Juan",
                        "Pedro",
        ...
        val dominicanSurnameArb: Arb<String> =
            arbitrary(edgecases = listOf("Pérez", "García", "Rodríguez", "Martínez")) {
                val surnames =
                    listOf(
                        "Pérez",
                        "García",
        ...
        "Property 14: Cédula OCR text parsing" -
            {
                ...
                "text with nombre and apellido extracts them" {
                    checkAll(100, dominicanSurnameArb, dominicanNameArb, cedulaNumberArb) {
                        apellido,
                        nombre,
                        cedulaNum ->
                        val lines =
