...
class AutoRoundConfig(QuantizationConfig):
    ...
    def apply_ipex_quant_layer(self, layer, prefix: str):
        # [PATCH] Explicit early exit for layers marked 16-bit in extra_config.
        # Fixes UR_RESULT_ERROR_DEVICE_LOST on XPU when GPTQLinearMethod
        # creates quantized weight tensors for MoE gate layers that should
        # remain in fp16. Checks both "prefix" and "model.prefix" forms.
        if self.extra_config and prefix:
            for cfg_key, cfg_val in self.extra_config.items():
                if (cfg_key == prefix
                        or cfg_key == f"model.{prefix}"
                        or f"model.{cfg_key}" == prefix):
                    if isinstance(cfg_val, dict) and cfg_val.get(
                            "bits", self.weight_bits) >= 16:
                        if isinstance(layer, (LinearBase, ParallelLMHead)):
                            return UnquantizedLinearMethod()
                        return None
        weight_bits, group_size, sym = self.get_layer_config(layer, prefix)
        ...
        if prefix and self.extra_config:
            for layer_name in self.extra_config:
                if (
                    ...
                    or f"model.{layer_name}" == prefix
                ) and self.extra_config[layer_name].get("bits", 16) >= 16:
                    logger.info("[PATCH] Skipping quantization for 16-bit layer: %s", prefix)
                    return UnquantizedLinearMethod()
        if "gate" in (prefix or ""):
            logger.warning("[PATCH-DEBUG] gate layer prefix=%s extra_config_keys=%s", prefix, list(self.extra_config.keys())[:3] if self.extra_config else None)
        if (
